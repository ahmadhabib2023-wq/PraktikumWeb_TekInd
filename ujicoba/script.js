...
formProduksi.addEventListener("submit", function (event) {
  ...
  if (jumlah > 100) {
    alert("Jumlah produksi tidak boleh lebih dari 100!");
    return;
  }

  // CEK DUPLIKAT (Tanggal, Operator, Shift sama)
  let dataLama = JSON.parse(localStorage.getItem(STORAGE_KEY)) || [];
  const isDuplicate = dataLama.some(
    (item) =>
  ...
  if (isDuplicate) {
    alert("Data dengan Tanggal, Operator, dan Shift yang sama sudah ada!");
    return;
  }
...
// SIMPAN DATA
// ==========================
function saveData(data, dataLama) {
  dataLama.push(data);
